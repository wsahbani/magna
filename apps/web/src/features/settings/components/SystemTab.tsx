...
export function SystemTab() {
  const { data: settings, isLoading } = useSettingsByCategory('system');
  ...
  if (!settings || settings.length === 0) {
    return (
      <div className="text-center py-12">
        <Body className="text-gray-500">Aucun paramètre système configuré</Body>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <Heading2>Paramètres Système</Heading2>
        <Button
          onClick={handleSave}
