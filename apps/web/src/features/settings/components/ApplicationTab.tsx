...
export function ApplicationTab() {
  const { data: settings, isLoading } = useSettingsByCategory('app');
  ...
  if (!settings || settings.length === 0) {
    return (
      <div className="text-center py-12">
        <Body className="text-gray-500">Aucun paramètre application configuré</Body>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <Heading2>Paramètres Application</Heading2>
        <Button
          onClick={handleSave}
