/**
 * Process Flow Store Hook Wrapper
...
import { useProcessFlowStore as useStore } from '../store/processFlowStore';
import { useProcessFlow, useSaveProcessFlow } from './useProcessFlow';
 ...
 * Hook return type
 */
export interface UseProcessFlowStoreReturn {
  // State
  nodes: Node[];
  edges: Edge[];
...
/**
 * Main hook for Process Flow Store
 * 
 * Features:
 ...
 */
export function useProcessFlowStore(
  processId: string | undefined,
): UseProcessFlowStoreReturn {
  ...
  useEffect(() => {
    if (flowData && processId) {
      // Set Process ID first
  ...
  const saveFlow = useCallback(async () => {
    if (!processId) {
      toast.error('Process ID manquant');
      return;
    }
  ...
  }, [processId, saveFlowMutation, markAsSaved]);
