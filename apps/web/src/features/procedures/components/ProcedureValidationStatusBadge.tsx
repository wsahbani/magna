...
interface ProcedureValidationStatusBadgeProps {
  procedureId: string
  compact?: boolean
}

export function ProcedureValidationStatusBadge({
  procedureId,
  compact = false,
}: ProcedureValidationStatusBadgeProps) {
  const { data: validationRequests = [], isLoading } = useProcedureValidationRequests(procedureId)
