...
import { useRequestProcedureValidation, useProcedureValidationRequests } from '../hooks/useProcedureValidation'
import type { Procedure } from '../types/procedure.types'

interface ProcedureValidationDialogProps {
  procedure: Procedure
  open: boolean
  onOpenChange: (open: boolean) => void
}

export function ProcedureValidationDialog({
  procedure,
  open,
  onOpenChange,
}: ProcedureValidationDialogProps) {
  ...
  const { data: validationRequests = [] } = useProcedureValidationRequests(procedure.id)
  const requestValidationMutation = useRequestProcedureValidation()
  ...
  const availableValidators = users.filter(
    (user) => user.id !== procedure.createdById,
  )
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Sélectionnez les utilisateurs qui doivent valider la procédure{' '}
