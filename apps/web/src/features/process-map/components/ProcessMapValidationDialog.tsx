...
import { useRequestProcessMapValidation, useProcessMapValidationRequests } from '../hooks/useProcessMapValidation'
import type { ProcessMap } from '../types/process-map.types'

interface ProcessMapValidationDialogProps {
  processMap: ProcessMap
  open: boolean
  onOpenChange: (open: boolean) => void
}

export function ProcessMapValidationDialog({
  processMap,
  open,
  onOpenChange,
}: ProcessMapValidationDialogProps) {
  const [selectedValidatorIds, setSelectedValidatorIds] = useState<string[]>([])
  const [comment, setComment] = useState('')
  const { data: users = [], isLoading: isLoadingUsers } = useUsers({ isActive: true })
  const { data: validationRequests = [] } = useProcessMapValidationRequests(processMap.id)
  const requestValidationMutation = useRequestProcessMapValidation()
  ...
  const handleSubmit = async () => {
    ...
    await requestValidationMutation.mutateAsync({
  ...
  const availableValidators = users.filter(
    (user) => user.id !== processMap.createdById,
  )
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Sélectionnez les utilisateurs qui doivent valider la carte de processus{' '}
            <span className="font-medium">{processMap.title}</span>
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4">
          ...
          <div>
            ...
            <Textarea
              ...
              placeholder="Ajouter un commentaire pour les validateurs..."
              rows={4}
